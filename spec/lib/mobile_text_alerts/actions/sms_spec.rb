...
RSpec.describe MobileTextAlerts::Actions::Sms do
  ...
  describe '#send_sms_to_number' do
    ...
    it 'raises a MobileTextAlert::Error on error response' do
      stub_request(:get, mta_client_url('send_message')).to_return(json_response('error'))
    ...
    it 'returns a hash on success response' do
      stub_request(:get, mta_client_url('send_message')).to_return(json_response('success'))
  ...
  describe '#send_sms_to_group' do
    let(:send_sms_to_group) { -> { client.send_sms_to_group('1', 'foo bar') } }

    it 'raises a MobileTextAlert::Error on error response' do
      stub_request(:get, mta_client_url('send_message')).to_return(json_response('error'))
      expect { send_sms_to_group.call }.to raise_error(MobileTextAlerts::Error, 'Error message is here')
    end

    it 'returns a hash on success response' do
      stub_request(:get, mta_client_url('send_message')).to_return(json_response('success'))
      response = send_sms_to_group.call
      expect(response['success']).to eq('Message Sent')
    end
